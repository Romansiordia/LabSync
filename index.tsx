...
const App: React.FC = () => {
  ...
  const [types, setTypes] = useState<AnalysisType[]>(() => {
    ...
    } catch { return SEED_TYPES; }
  });

  const [analyses, setAnalyses] = useState<AnalysisRecord[]>(() => {
    try { return JSON.parse(localStorage.getItem('lab_analyses') || '[]'); } catch { return []; }
  });
  ...
  const handleResetSystem = () => {
    if (confirm("¿Estás seguro de restablecer los datos de catálogo?")) {
      setClients(SEED_CLIENTS);
      setTechs(SEED_TECHS);
      setTypes(SEED_TYPES);
      alert("Catálogos de fábrica cargados.");
    }
  };
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden text-slate-800 font-sans">
      <aside className="w-72 bg-white border-r border-slate-200 p-8 flex flex-col gap-8 shadow-sm shrink-0">
      ...
      <main className="flex-1 overflow-y-auto relative">
        <header className="sticky top-0 bg-white/90 backdrop-blur-md border-b border-slate-200 px-10 py-6 flex justify-between items-center z-20">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="bg-indigo-50 px-4 py-2 rounded-xl text-indigo-600 font-black text-xs border border-indigo-100 uppercase">Sistema Activo</div>
          </div>
        </header>
        ...
        <div className="p-10 animate-in">
            ...
            <div className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="bg-white p-7 rounded-[2rem] border border-slate-100 shadow-sm">
                  ...
                  <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest">Muestras</p>
                ...
                <div className="bg-white p-7 rounded-[2rem] border border-slate-100 shadow-sm">
                  ...
                  <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest">En Análisis</p>
                ...
                <div className="bg-white p-7 rounded-[2rem] border border-slate-100 shadow-sm">
                  ...
                  <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest">Proyección</p>
                  <h3 className="text-3xl font-black text-slate-900">${analyses.reduce((s, a) => s + a.cost, 0).toLocaleString()}</h3>
                </div>
              </div>
              <div className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm">
                <h4 className="text-xl font-black mb-8">Volumen de Trabajo</h4>
                <div className="h-64">
                   <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={analyses.length > 0 ? analyses.slice(-7).reverse() : [{Folio: 'S/D', cost: 0}]}>
          ...
          {activeTab === 'analysis' && (
            <div className="space-y-6">
              <div className="flex justify-between items-center bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm">
                <div>
                  <h3 className="text-xl font-black text-slate-900">Bitácora Bromatológica</h3>
                  <p className="text-slate-400 text-[10px] font-black uppercase mt-1">Gestión remota de muestras</p>
                </div>
                <button onClick={() => setShowAddModal(true)} className="bg-indigo-600 text-white px-10 py-5 rounded-[1.5rem] font-black flex items-center gap-3 hover:scale-[1.02] transition-all shadow-lg"><Plus size={20}/> Nueva Muestra</button>
              </div>
              <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 overflow-x-auto">
                <table className="w-full text-left border-collapse min-w-[800px]">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {analyses.length === 0 ? (
                      <tr><td colSpan={5} className="py-20 text-center text-slate-300 font-bold uppercase italic">Sin registros actuales</td></tr>
                    ) : (
                      analyses.map(a => (
                        <tr key={a.id} className="hover:bg-slate-50/50 transition-colors">
                          ...
                          <td className="px-10 py-6">
                            ...
                            <div className="text-[10px] text-indigo-500 font-black uppercase">{a.product}</div>
                          </td>
                          <td className="px-10 py-6">
          ...
          {activeTab === 'clients' && (
            <div className="space-y-8">
               <div className="flex justify-between items-center bg-white p-8 rounded-3xl border border-slate-100 shadow-sm">
                  <h3 className="text-xl font-black">Empresas / Clientes</h3>
                  <button onClick={() => {
                     const n = prompt("Nombre Comercial:");
                     if(n) setClients([{ id: 'c'+Date.now(), name: n, contactName: 'Gerente de Calidad', email: 'lab@correo.com', phone: '555-0000', address: '' }, ...clients]);
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {clients.map(c => (
                  <div key={c.id} className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-xl transition-all relative group">
                    ...
                    <p className="text-slate-400 text-[10px] font-black uppercase mb-4">{c.contactName}</p>
          ...
          {activeTab === 'techs' && (
             <div className="space-y-8">
               <div className="flex justify-between items-center bg-white p-8 rounded-3xl border border-slate-100 shadow-sm">
                  <h3 className="text-xl font-black">Cuerpo de Analistas</h3>
                  <button onClick={() => {
                     const n = prompt("Nombre del Técnico:");
                     if(n) setTechs([{ id: 't'+Date.now(), name: n, specialty: 'Analista Bromatólogo', email: 'staff@lab.com' }, ...techs]);
                  }} className="bg-emerald-600 text-white px-8 py-3 rounded-2xl font-black flex items-center gap-2 shadow-lg"><Plus size={20}/> Registrar Técnico</button>
               </div>
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {techs.map(t => (
                    <div key={t.id} className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm flex items-center gap-6 relative group">
                       ...
                       <div className="bg-emerald-50 text-emerald-600 w-16 h-16 rounded-[1.5rem] flex items-center justify-center font-black text-2xl uppercase shrink-0">{t.name.split(' ').map(n => n[0]).join('')}</div>
                       <div>
          ...
          {activeTab === 'types' && (
            <div className="space-y-6">
              <div className="flex justify-between items-center bg-white p-8 rounded-3xl border border-slate-100 shadow-sm">
                  <h3 className="text-xl font-black">Servicios Disponibles</h3>
                  <button onClick={() => {
                    const n = prompt("Nombre del Análisis:");
                    const c = prompt("Costo Base:");
                    if(n) setTypes([{ id: 'at'+Date.now(), name: n, baseCost: parseFloat(c || "0"), unit: '%' }, ...types]);
                  }} className="bg-slate-900 text-white px-8 py-3 rounded-2xl font-black flex items-center gap-2 shadow-lg hover:bg-black transition-all"><Plus size={20}/> Nuevo Estudio</button>
              </div>
              <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden overflow-x-auto">
                <table className="w-full text-left min-w-[600px]">
                  <thead><tr className="bg-slate-50 border-b border-slate-200 text-[10px] font-black text-slate-400 uppercase tracking-widest"><th className="px-10 py-6">Parámetro Bromatológico</th><th className="px-10 py-6">Unidad</th><th className="px-10 py-6 text-right">Costo Estándar</th><th className="px-10 py-6 text-right">Acción</th></tr></thead>
                  <tbody className="divide-y divide-slate-100">
                    {types.map(t => (
          ...
          {activeTab === 'settings' && (
            <div className="max-w-xl mx-auto space-y-8">
              <div className="bg-white p-12 rounded-[3rem] shadow-sm border border-slate-100 flex flex-col items-center text-center">
                ...
                <h3 className="text-xl font-black mb-4">Sincronización Cloud</h3>
                <p className="text-slate-400 text-sm mb-10 font-medium px-4">Introduce la URL de tu Web App de Google para activar el volcado de datos.</p>
                <input type="text" className="w-full px-6 py-4 bg-slate-50 rounded-2xl border border-slate-200 font-mono text-xs focus:ring-4 focus:ring-indigo-100 outline-none transition-all mb-4" value={googleUrl} onChange={(e) => setGoogleUrl(e.target.value)} placeholder="https://script.google.com/macros/s/..." />
                <button onClick={() => alert("Configuración guardada.")} className="w-full py-5 bg-indigo-600 text-white font-black rounded-2xl shadow-xl hover:bg-indigo-700 transition-all uppercase tracking-widest text-xs">Guardar Conexión</button>
              </div>
              <div className="bg-white p-10 rounded-[2.5rem] border border-slate-100 flex items-center justify-between">
                 <div className="flex items-center gap-4 text-left">
                    ...
                    <div>
                       <h4 className="font-black text-slate-900">Limpieza de Caché</h4>
                       <p className="text-xs text-slate-400 font-medium">Restablece catálogos a valores iniciales de fábrica.</p>
                    </div>
                 </div>
                 <button onClick={handleResetSystem} className="flex items-center gap-2 px-6 py-3 bg-slate-100 text-slate-600 rounded-2xl font-black text-xs uppercase hover:bg-red-600 hover:text-white transition-all"><RotateCcw size={16}/> Restablecer</button>
              </div>
            </div>
          )}
        </div>
          ...
          <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-md z-[100] flex items-center justify-center p-6 animate-in">
            <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-4xl overflow-hidden border border-white/20">
              <div className="p-10 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
                <h3 className="text-2xl font-black">Registro de Muestra</h3>
              ...
              <form onSubmit={handleCreateAnalysis} className="p-10 space-y-10">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  <div className="space-y-6">
                    <input name="sampleName" required placeholder="Identificador / Lote *" className="w-full px-8 py-4 bg-slate-50 rounded-2xl border-none font-black outline-none focus:ring-4 focus:ring-indigo-100" />
                    ...
                    <select name="techId" required className="w-full px-8 py-4 bg-slate-50 rounded-2xl border-none font-black outline-none">
                       <option value="">Analista Asignado...</option>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="px-4"><label className="text-[10px] font-black text-slate-400 uppercase">Compromiso de Entrega</label>
                ...
                <div className="p-8 bg-slate-50 rounded-[2rem] border border-slate-100 shadow-inner">
                  <h5 className="text-[10px] font-black text-slate-400 uppercase mb-6 tracking-widest flex items-center gap-2"><Beaker size={14}/> Análisis Solicitados</h5>
                  <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                    {types.map(t => (
                      <label key={t.id} className="flex items-center gap-3 p-4 bg-white border border-slate-200 rounded-xl cursor-pointer hover:border-indigo-400 transition-all font-black text-[10px] has-[:checked]:border-indigo-600 has-[:checked]:bg-indigo-50 shadow-sm uppercase tracking-tighter">
                        <input type="checkbox" name={`type-${t.id}`} className="w-5 h-5 accent-indigo-600 rounded-lg shrink-0" />
                        <span className="truncate">{t.name}</span>
                      </label>
                    ))}
                  </div>
                </div>
                <button type="submit" className="w-full py-6 bg-indigo-600 text-white rounded-[2rem] font-black shadow-2xl hover:bg-indigo-700 transition-all uppercase tracking-widest text-xs flex items-center justify-center gap-3"><Save size={18}/> Guardar y Enviar a Nube</button>
              </form>
            </div>
          </div>
        )}
          ...
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[110] flex items-center justify-center p-6 animate-in">
            <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-2xl overflow-hidden border border-white/20">
               <div className="p-10 bg-indigo-600 text-white flex justify-between items-center shadow-lg">
                  <div><h3 className="text-2xl font-black">Captura de Datos</h3><p className="font-bold text-indigo-100 text-[10px] uppercase mt-1">MUESTRA: {selectedRecord.Folio} | {selectedRecord.sampleName}</p></div>
               ...
               <div className="p-12 space-y-8">
                  ...
                  <button onClick={handleSaveResults} className="w-full py-6 bg-slate-900 text-white rounded-[2rem] font-black shadow-2xl hover:bg-slate-800 transition-all flex items-center justify-center gap-3 uppercase tracking-widest text-xs"><CheckCircle size={18}/> Finalizar Análisis</button>
               </div>
            </div>
          </div>
        )}
